...
/* 超过最大内存块1024, 就分配页框 */
    if (size > 1024) {
        ...
        /* 开始分配内存块 */
        b = elem2entry(
                struct mem_block, free_elem, list_pop(&(descs[desc_idx].free_list)));
...
void pfree(uint32_t pg_phy_addr) {
    ...
    bitmap_set(&mem_pool->pool_bitmap, bit_idx, 0);//将位图指定位置置0
}

void page_table_pte_remove(uint32_t vaddr) {
    uint32_t *pte = pte_ptr(vaddr);
    *pte &= ~PG_P_1;
    asm("invlpg %0"::"m"(vaddr):"memory");
}

//从指定虚拟内存池中，连续删除pg_cnt个虚拟内存页地址
void vaddr_remove(enum pool_flags poolFlags, void *vaddr, uint32_t pg_cnt) {
    struct virtual_addr *virtualAddr;//虚拟内存池
    if (poolFlags == PF_KERNEL) {
